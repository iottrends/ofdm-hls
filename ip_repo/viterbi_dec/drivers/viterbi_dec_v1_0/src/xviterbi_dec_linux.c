...
typedef struct {
    ...
    XViterbi_dec_uio_map maps[ MAX_UIO_MAPS ];
} XViterbi_dec_uio_info;

/***************** Variable Definitions **************************************/
static XViterbi_dec_uio_info uio_info;
...
static int uio_info_read_name(XViterbi_dec_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XViterbi_dec_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XViterbi_dec_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XViterbi_dec_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XViterbi_dec_Initialize(XViterbi_dec *InstancePtr, const char* InstanceName) {
	XViterbi_dec_uio_info *InfoPtr = &uio_info;
