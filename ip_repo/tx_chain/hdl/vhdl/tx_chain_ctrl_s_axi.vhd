...
end entity tx_chain_ctrl_s_axi;
...
--        others - reserved
-- 0x10 : Data signal of n_data_bytes
--        bit 15~0 - n_data_bytes[15:0] (Read/Write)
--        others   - reserved
-- 0x14 : reserved
-- 0x18 : Data signal of modcod
--        bit 1~0 - modcod[1:0] (Read/Write)
--        others  - reserved
-- 0x1c : reserved
-- 0x20 : Data signal of n_syms
--        bit 7~0 - n_syms[7:0] (Read/Write)
--        others  - reserved
-- 0x24 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of tx_chain_ctrl_s_axi is
...
attribute DowngradeIPIdentifiedWarnings of behave : architecture is "yes";
    ...
    constant ADDR_N_DATA_BYTES_DATA_0 : INTEGER := 16#10#;
    constant ADDR_N_DATA_BYTES_CTRL   : INTEGER := 16#14#;
    constant ADDR_MODCOD_DATA_0       : INTEGER := 16#18#;
    constant ADDR_MODCOD_CTRL         : INTEGER := 16#1c#;
    constant ADDR_N_SYMS_DATA_0       : INTEGER := 16#20#;
    constant ADDR_N_SYMS_CTRL         : INTEGER := 16#24#;
    ...
    signal int_n_data_bytes    : UNSIGNED(15 downto 0) := (others => '0');
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_N_DATA_BYTES_DATA_0 =>
                        rdata_data <= RESIZE(int_n_data_bytes(15 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_n_data_bytes(15 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_N_DATA_BYTES_DATA_0) then
                    int_n_data_bytes(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_n_data_bytes(15 downto 0));
                end if;
            end if;
        end if;
    end process;
