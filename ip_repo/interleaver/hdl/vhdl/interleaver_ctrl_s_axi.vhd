...
entity interleaver_ctrl_s_axi is
generic (
...
end entity interleaver_ctrl_s_axi;
...
architecture behave of interleaver_ctrl_s_axi is
...
attribute DowngradeIPIdentifiedWarnings of behave : architecture is "yes";
    ...
    constant ADDR_IS_RX_DATA_0  : INTEGER := 16#20#;
    constant ADDR_IS_RX_CTRL    : INTEGER := 16#24#;
    ...
    signal int_is_rx           : UNSIGNED(0 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_IS_RX_DATA_0 =>
                        rdata_data <= RESIZE(int_is_rx(0 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    is_rx                <= STD_LOGIC_VECTOR(int_is_rx);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_is_rx(0 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_IS_RX_DATA_0) then
                    int_is_rx(0 downto 0) <= (UNSIGNED(WDATA(0 downto 0)) and wmask(0 downto 0)) or ((not wmask(0 downto 0)) and int_is_rx(0 downto 0));
                end if;
            end if;
        end if;
    end process;
