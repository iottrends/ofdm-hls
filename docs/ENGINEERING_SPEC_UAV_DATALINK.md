...
**Date:** 2026-04-15 (rev. 4 — sync_detect sliding-window FSM, sync_cfo retired, RX triad now ap_ctrl_none)
...
| Stage | HLS module | Function |
|-------|------------|----------|
| Timing sync + CFO | `sync_detect` **v4** (formerly merged `sync_cfo`; `sync_cfo` retired as tombstone) | **Free-running 4-state FSM** (`SEARCH → FWD_PREHDR → WAIT_NSYMS → FWD_DATA → SEARCH`) running on a 4096-sample circular buffer with per-cycle sliding-window running sums for P, R, Rl, and pow_env (64-sample window). **Dual-threshold trigger:** AND of (`pow_env > POW_TH`, s_axilite-tunable, CSR-readback `stat_pow_env`) and (`\|P\|² > SC_TH² · R · Rl`, SC_TH² = 0.49 as ap_ufixed<16,0>, cross-multiply — no divide). **Inline CFO derotation** via 256-entry quarter-wave sin LUT — `cfo_correct.cpp` is no longer part of the build. Back-pressure-safe output via `iq_out.write_nb()` — only `rd_ptr` / `fwd_remaining` advance on successful write, while upstream samples continue filling the circular buffer unconditionally. 4096-sample warmup counter after reset (`gate_armed` latches high once and never clears, shared with the 288-sample deaf-window mechanism used on `FWD_DATA → SEARCH`). On that re-entry, P / R / Rl / pow_env accumulators and the 64-deep pow_dly shift register (FF, complete-partitioned) are all zeroed in one cycle. Feedback from `ofdm_rx` is the automatic `n_syms_fb_ap_vld` companion wire, sticky-latched in WAIT_NSYMS; `n_syms_fb = 0` aborts to SEARCH, `n_syms_fb > 0` sets `fwd_remaining = n_syms × 288` and transitions to FWD_DATA. |
| Coarse CFO | `sync_cfo` | ε_sc = arg(P[t*]) / 2π, \|ε_sc\| ≤ 0.5 subcarrier |
| CFO correct | `cfo_correct` | Per-sample derotation e^(−j2πε_sc n/256), 256-entry quarter-wave sin/cos LUT |
...
- **Control-plane split (TX vs RX):**
  ...
  - **RX path is entirely free-running.** A single `xlconstant rx_ap_start_hi = 1` drives `ap_start` on `sync_cfo`, `ofdm_rx`, `fec_rx`, and `ofdm_mac`. These blocks auto-start from reset. The MAC has **zero** write access to RX PHY CSRs (the `rx_arm_phy()` helper and the `PHY_BASE_SYNC_CFO / OFDM_RX / FEC_RX` constants have been deleted).
  - Per-packet RX sequencing is done entirely in hardware:
    `ofdm_rx` decodes the BPSK header → drives `modcod_out` / `n_syms_out` as ap_none wires into `fec_rx`, and pulses `n_syms_fb` (ap_vld) back to `sync_cfo` (n_syms on good header → forward that many data symbols; 0 on header-CRC error → return to SEARCH cleanly, no drain loop needed).
- Top-level free-running bodies:
  - `ofdm_rx`: `FREE_RUN: while(1) { … }`
  - `fec_rx` : `FREE_RUN: while(1) { #pragma HLS DATAFLOW … }`
  - `sync_cfo`: keeps its current DATAFLOW wrap of `sync_detect + cfo_correct` until the sliding-window rewrite lands.
- Merged HLS IPs (reduce Vivado BD IP count):
  - `tx_chain`    = scrambler + conv_enc + interleaver (one DATAFLOW block)
  - `sync_detect` **v4** = sliding-window FSM with inline CFO derotation (CFO no longer a separate IP; `cfo_correct.cpp` dropped from build)
...
|----------|------|-----------|----|
| Slice LUT | 15 395 | 32 600 | **47 %** |
| Slice FF  | 19 940 | 65 200 | 31 % |
| DSP48E1   |     94 |    120 | **78 %** *(tightest)* |
| BRAM tile |     26 |     75 | 35 % |
| Bonded IOB|     82 |    150 | 55 % |

All paths meet timing at 100 MHz (main) and 200 MHz (FEC) with ≈ 10 % slack margin in Vivado 2025.2. DSP is the binding resource and sets the practical ceiling for future features (e.g., soft-decision metric, multi-tap equaliser).

---

## 11. Performance Summary

**Throughput (net, 255 symbols/frame, 20 MSPS):**
- QPSK r1/2 — **≈ 1.39 Mbps**
- QPSK r2/3 — ≈ 1.85 Mbps
- 16-QAM r1/2 — ≈ 2.78 Mbps
- 16-QAM r2/3 — **≈ 4.17 Mbps**
...
- **`sync_detect` v4 co-sim pending.** Rewrite is complete (4-state FSM, sliding-window running sums, dual-threshold detection, inline CFO derotation, back-pressure-safe `write_nb` output, 4096-sample warmup + shared 288-sample post-packet deaf window, full accumulator + pow_dly reset on `FWD_DATA → SEARCH`). Verification plan: (i) sync_detect-alone co-sim with one good packet + one bad-CRC header + 50 consecutive bad-CRC headers to stress the silent-deadlock regression path; (ii) CFO phase-step scaling sanity-check against the retired `cfo_correct` (the current `(ap_int<20>)(ang.range()) << 12` expression preserves sign and range but the fractional-bit count needs analytical verification); (iii) full loopback with `sync_detect` in the chain. Known co-sim risks: II=1 schedule closure (5 BRAM reads/cycle is borderline — may need duplicated I/Q buffers into 8 BRAM18), CRC-fail silent-deadlock, back-pressure during FWD states, warmup-gate correctness.
- Hard-decision Viterbi — soft-decision upgrade pending (+2 – 3 dB).
- One-shot (preamble-only) channel estimate — adequate for slow-fading UAV LOS channels; a pilot-aided tracking filter is feasible within remaining DSP budget.
- Half-duplex MAC only; FDD would double RF-chain cost.
...
| Sync + CFO | `src/sync_detect.{cpp,h}` (v4, inline CFO). `src/sync_cfo.{cpp,h}` retained as tombstone stub; `src/cfo_correct.{cpp,h}` no longer in build. `src/free_run.h` — shared `FREE_RUN_LOOP_*` macros and `S_AXILITE_STAT_BUNDLE`. |
